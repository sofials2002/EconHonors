...
household_data <- read_csv("/Users/sofialozano/Desktop/EconHonors/Data/Raw/Honors_Thesis_Data/UNHCR_LBN_2020_VASYR_data/UNHCR_LBN_2020_VASYR_data_household_v2.1.csv")
#Load the individual data:
individual_data <- read_csv("/Users/sofialozano/Desktop/EconHonors/Data/Raw/Honors_Thesis_Data/UNHCR_LBN_2020_VASYR_data/UNHCR_LBN_2020_VASYR_data_individual_v2.1.csv")

#Check the data:
head(household_data)
head(individual_data)

#see variables with missing values:
colSums(is.na(household_data))
#sort the variables by the number of missing values:
...
unique(individual_data$highest_grade_t)
#mutate into highest_grade:
individual_data <- individual_data %>% mutate(highest_grade = highest_grade_t)

#remove the first lowercase letters and the periods:
individual_data$highest_grade <- gsub("^[a-z]. ", "", individual_data$highest_grade)
...
individual_data$highest_grade_category[individual_data$highest_grade %in% c("Nursery","Kindergarten 1","Kindergarten 2")] <- "Kindergarten"
...
individual_data$highest_grade_category[individual_data$highest_grade %in% c("Grade 7", "Grade 8", "Grade 9", "Grade 10", "Grade 11", "Grade 12","TVET: Grade 7", "TVET: Grade 8", "TVET: Grade 9", "TVET: Grade 10", "TVET: Grade 11", "TVET: Grade 12")] <- "Secondary"
individual_data$highest_grade_category[individual_data$highest_grade == "University"] <- "University"
individual_data$highest_grade_category[individual_data$highest_grade %in% c("Technical course", "TVET License Technique")] <- "Technical course"
individual_data$highest_grade_category[individual_data$highest_grade == "Did not attend school"] <- "Illiterate"
individual_data$highest_grade_category[individual_data$highest_grade %in% c("Dont Know", "NA")] <- NA
...
table(individual_data$children) #This means 13,121 individuals are children and 8897 are not children.
...
table(individual_data$children_under_5) #This means 4280 individuals are children under 5 years old.
...
summary(individual_data$rel_to_hoh_s)

#mutate into hoh_relation:
...
relation_mapping <- c("a. Head of Household" = 1,
                      ...
                      "d. Daughter/Son", "e. Brother/Sister" = 3,
                     "NA" = NA,
                      "f. Father-in-law/Mother-in-law" = 5,
                      "h. Extended family (uncle/aunt/cousin/niece/nephew etc)" = 6,
                      "g. Brother-in-law/Sister-in-law" = 7,
                      "h. No family relationship guest", "i. Other"  = 8)
...
individual_data <- individual_data %>% mutate(is_head_of_household = ifelse(hoh_relation == "a. Head of Household", 1, 0))

#how many 1s and 0s are in the is_head_of_household variable?
table(individual_data$is_head_of_household) #This means 4448 individuals are head of household.

#create the variable has_spouse, from the civil_status variable:
individual_data$has_spouse <- ifelse(individual_data$marital_status_s == "Married (MA)", 1, 0)

#Create a numeric variable called legal_residency_numeric from the legal_residency variable:
individual_data$legal_residency_numeric <- ifelse(individual_data$legal_res_yn == "a. Yes", 1, 0)

#convert to dummies the temp_illness, chronic_illness, serious_med_cond and mental_illness variables:
#use grepl to find the variables that contain the word ill:
names(individual_data)[grepl("health", names(individual_data))]

#serious_med_cond: "dis_dif_walking_s", "dis_dif_hearing_s", "dis_dif_seeing_s", "dis_dif_hands_s", "dis_dif_rais_jug_s" 
...
individual_data$sex_dummy <- ifelse(individual_data$gender_s == "b. Female", 1, 0)
...
#WORK REGULARLY: 
#use grepl to see which variables have the name work:
names(individual_data)[grepl("work", names(individual_data))]

#create work_regular_dummy variable: work_as_s: 
...
table(individual_data$work_regular_dummy) #3196 people work regularly. 
...
#clean the variables:
#use grepl to see which variables have the name food:
...
#Remove the lowercase letters followed by a period in the district_s variable:
household_data$district_s <- gsub("^[a-z]\\.", "", household_data$district_s)

#print the unique districts:
print(unique(household_data$district_s))

#Remove the leading and trailing whitespace in the district_s variable:
household_data$district_s <- trimws(household_data$district_s)

#print the unique districts:
print(unique(household_data$district_s))

unique(household_data$district_s)

location_mapping <- list(
  "Akkar" = c("Akkar"),
  "North" = c("Zgharta", "Bcharre", "El Minieh Dennie", "Tripoli", "El Koura", "Kesrwane", "Jbeil", "Bent Jbeil"),
  "Mount Lebanon" = c("El Meten", "Baabda", "Aley", "Chouf", "El Batroun"),
  "Beirut" = c("Beirut"),
  "Baalbek-El Hermel" = c("Baalbek", "El Hermel"),
  "Bekaa" = c("Rachaya", "Zahle", "West Bekaa"),
  "South" = c("Saida", "Sour", "Jezzine"),
  "El Nabatieh" = c("El Nabatieh", "Hasbaya", "Marjaayoun")
)
# Create a new variable 'governorate' based on the mapping:
household_data$governorate <- NA
for (i in seq_along(location_mapping)) {
  household_data$governorate[household_data$district_s %in% location_mapping[[i]]] <- names(location_mapping)[i]
}

#print the unique governorates:
unique_governorates <- unique(household_data$governorate)
print(unique_governorates)

table(household_data$governorate)

#see how many nas:
sum(is.na(household_data$governorate))

#create 8 dummies one for each governorate:
household_data$Akkar_dummy <- ifelse(household_data$governorate == "Akkar", 1, 0)
household_data$BaalbekHermel_dummy <- ifelse(household_data$governorate == "Baalbek-El Hermel", 1, 0)
household_data$Beirut_dummy <- ifelse(household_data$governorate == "Beirut", 1, 0)
household_data$Beqaa_dummy <- ifelse(household_data$governorate == "Bekaa", 1, 0)
household_data$MountLebanon_dummy <- ifelse(household_data$governorate == "Mount Lebanon", 1, 0)
household_data$North_dummy <- ifelse(household_data$governorate == "North", 1, 0)
household_data$South_dummy <- ifelse(household_data$governorate == "South", 1, 0)
household_data$Nabatieh_dummy <- ifelse(household_data$governorate == "El Nabatieh", 1, 0)

#from the list above, I excluded Keserwan as a governorate. Might have to compare with other years later. 
#coping mechanisms:
...
#crisis_coping
names(household_data)[grepl("mari", names(household_data))]
table(household_data$withd_chld_school_yn)

table(household_data$withd_chld_school_yn)
table(household_data$red_non_food_exp_edu_yn)
table(household_data$red_non_food_exp_health_yn)
table(household_data$child_mariage_yn)

#create a dummy for each of the variables in the list:

# List of variables you want to apply the operation to
variables_to_process <- c("withd_chld_school_yn", "red_non_food_exp_edu_yn", "red_non_food_exp_health_yn", "child_mariage_yn")
...
table(household_data$child_mariage_yn_dummy)

#create crisis_coping
household_data$crisis_coping <- ifelse(household_data$withd_chld_school_yn_dummy == 1 | 
                                         household_data$red_non_food_exp_edu_yn_dummy == 1 |
                                         household_data$red_non_food_exp_health_yn_dummy == 1 |
                                         household_data$child_mariage_yn == 1, 1, 0)
...
household_data$sold_house <- household_data$sold_house_yn
#mutate sold_assets_yn to selling_assets:
household_data$selling_assets <- household_data$sold_assets_yn
#mutate child_exploit_work_yn to child_exploitative_work:
household_data$child_exploitative_work <- household_data$child_exploit_work_yn
#mutate begging_yn to begging:
household_data$begging <- household_data$begging_yn
#mutate child_labour_yn to child_labour:
household_data$child_labour <- household_data$child_labour_yn
...
#borrow_money_src_m_frnd_leb // borrow_sourcefriends_leb #3018 households. 
#borrow_money_src_m_frnd_no_leb // borrow_sourcefriends_not_leb #91 households. 
#borrow_money_src_m_loc_charity // borrow_sourcelocal_charity #9 households. 
#prim_reason_credit_m_food // reason_borrowingfood #2975 households. 
...
#assets:
dic <- read.csv("/Users/sofialozano/Desktop/EconHonors/Data/Raw/Honors_Thesis_Data/UNHCR_LBN_2020_VASYR_data/UNHCR_LBN_2020_VASYR_data_household_labels.csv")
...
variables_to_process <- c("cleaning_items_yn", "female_hygiene_yn", "baby_care_yn",
                          ...
                          "electricity_access_yn", "sms_yn", "wash_drinking_yn" )
...
#summary(household_data$expenditure_transport)
#create transport_dummy if household spent more than 9856 on transport:
#household_data$transport_dummy <- ifelse(household_data$expenditure_transport > 9856, 1, 0)
#create clothing_dummy if household spent more than 4078 on clothing:
#household_data$clothing_dummy <- ifelse(household_data$expenditure_clothing > 4078, 1, 0)
...
variables_to_process <- c("cleaning_items_yn_dummy", "female_hygiene_yn_dummy", "baby_care_yn_dummy",
                          ...
                          "electricity_access_yn_dummy", "transport_dummy", "clothing_dummy")
...
names(household_data)[grepl("win", names(household_data))]

#products variables: 
table(household_data$pers_hygiene_yn_dummy)
table(household_data$female_hygiene_yn)
table(household_data$cleaning_items_yn_dummy)
table(household_data$baby_care_yn)

#mobile phone, internet, electricity access, wifi, make a table
table(household_data$have_internet_phone_yn_dummy)
table(household_data$have_internet_wifi_yn_dummy)
...
#turn female_hygiene_yn into a dummy:
household_data$female_hygiene_yn_dummy <- ifelse(household_data$female_hygiene_yn == "a.       Yes ", 1, 0)
#turn baby_care_yn into a dummy:
household_data$baby_care_yn_dummy <- ifelse(household_data$baby_care_yn == "a.       Yes ", 1, 0)
#turn electricity_access_yn into a dummy:
household_data$electricity_access_yn_dummy <- ifelse(household_data$electricity_access_yn == "a. Yes", 1, 0)

#turn leaking_roof_yn into a dummy:
household_data$leaking_roof_yn_dummy <- ifelse(household_data$leaking_roof_yn == "a. Yes", 1, 0)
#turn damaged_shelter_yn into a dummy:
household_data$damaged_shelter_yn_dummy <- ifelse(household_data$damaged_shelter_yn == "a. Yes", 1, 0)
#turn damaged_roof_yn into a dummy:
household_data$damaged_roof_yn_dummy <- ifelse(household_data$damaged_roof_yn == "a. Yes", 1, 0)
#turn damage_walls_yn into a dummy:
household_data$damage_walls_yn_dummy <- ifelse(household_data$damage_walls_yn == "a. Yes", 1, 0)
#turn water_pipes_not_func_yn into a dummy:
household_data$water_pipes_not_func_yn_dummy <- ifelse(household_data$water_pipes_not_func_yn == "a. Yes", 1, 0)
#turn rottenness_leak_yn into a dummy:
household_data$rottenness_leak_yn_dummy <- ifelse(household_data$rottenness_leak_yn == "a. Yes", 1, 0)
#turn unsealed_win_doors_yn into a dummy:
household_data$unsealed_win_doors_yn_dummy <- ifelse(household_data$unsealed_win_doors_yn == "a. Yes", 1, 0)
...
household_data$communication_assets <- rowSums(household_data[, c("electricity_access_yn_dummy", "have_internet_wifi_yn_dummy")])
...
# household_data1$total_assets <- rowSums(household_data1[, 2:10], na.rm = TRUE)
# 
...
# household_data$communication_assets <- rowSums(household_data[, c("have_internet_phone_yn_dummy", "have_internet_wifi_yn_dummy")])
# 
...
#water expenditure: 
#expenditure_water
table(household_data$wash_drinking_yn_dummy)
#mutate to expenditure_water_dummy
household_data$expenditure_water_dummy <- household_data$wash_drinking_yn_dummy

#household expenditure:
#summary of the variable:
table(household_data$expenditure_household)
table(household_data$exp_total_dec)

#see nas in the variable:
sum(is.na(household_data$expenditure_household))
...
#use grepl to see which variables have the name cons:
names(household_data)[grepl("spice", names(household_data))]

household_data$FCScore <- (
  ifelse(is.na(household_data$num_days_tubers_cons_i), 0, household_data$num_days_tubers_cons_i) * 3 +
...
table(household_data$FCScore) #the FCScore is between 0 and 101. There's an issue with the spices, but it isn't bad. 
summary(household_data$FCScore) #i'm getting a lot of nas. 

#curfew variables:
#table(household_data$curfews_yn)
#unique(household_data$curfews_yn)
household_data$curfew_dummy <- ifelse(household_data$curfews_yn == "a. Yes", 1, 0)

#technology acceess:
#use grepl to find sms
names(household_data)[grepl("sms", names(household_data))]
#mutate sms_yn_dummy to info_servicessms_dummy
household_data$info_servicessms_dummy <- household_data$sms_yn_dummy
table(household_data$info_servicessms_dummy) #most get their info from sms. 3801.
...
names(household_data)[grepl("safety", names(household_data))]
...
names(household_data)[grepl("6", names(household_data))]
table(household_data$accom_living_period)
summary(household_data$accom_living_period)
#this variable will show wether the person has an unstable living situation, or recently had. Most people have been living in the same place for more than 6 months.
household_data$accom_short_living_period_dummy <- ifelse(household_data$changed_accom_yn == "a. Yes" | 
                                                           household_data$move_accom_yesno == "b. No", 1, 0)
...
#health required: prim_health_care_yn
#health access: prim_health_case_ass_yn
...
table(household_data$main_income_src_s)

household_data$ecard_dummy <- ifelse(
  household_data$main_income_src_s == "i. E-cards used in WFP FOOD SHOPS" |
    household_data$sec_income_src_s == "i. E-cards used in WFP FOOD SHOPS" |
    household_data$third_income_src_s == "i. E-cards used in WFP FOOD SHOPS",
  1, 0)
...
#child labor variable:
table(household_data$child_labour_dummy)

#include: 
names(household_data)[grepl("food", names(household_data))]
...
household_data1 <- household_data %>% select (formid, household_size, governorate, Akkar_dummy, BaalbekHermel_dummy, Beirut_dummy, Beqaa_dummy, MountLebanon_dummy, 
                                              ...
                                              appliance_assets, communication_assets, mobile_phone_dummy, expenditure_water_dummy, FCScore, rCSI,
...
#transportation assets, excluded

#mutate expenditure_water_dummy to spent_on_water
household_data1$spent_on_water <- household_data1$expenditure_water_dummy
