...
household_data <- read_csv("/Users/sofialozano/Desktop/EconHonors/Data/Raw/Honors_Thesis_Data/UNHCR_LBN_2022_VASYR_data_v2_1/UNHCR_LBN_2022_VASYR_data_household_v2.1.csv")
#Load the individual data:
individual_data <- read_csv("/Users/sofialozano/Desktop/EconHonors/Data/Raw/Honors_Thesis_Data/UNHCR_LBN_2022_VASYR_data_v2_1/UNHCR_LBN_2022_VASYR_data_member_v2.1.csv")
...
individual_data <- individual_data %>% mutate(formid = id)
household_data <- household_data %>% mutate(formid = id)
...
#use grepl to find illness in variable names: 
names(individual_data)[grepl("walk", names(individual_data))]

# Create dummy variables for dis_dif_hearing_s
individual_data$dis_dif_hearing_dummy <- ifelse(individual_data$dis_dif_hearing_s %in% c("c.A lot of difficulty", "b.Some difficulty", "d.Cannot do at all"), 1, 0)
# Create dummy variables for dis_dif_seeing_s
individual_data$dis_dif_seeing_dummy <- ifelse(individual_data$dis_dif_seeing_s %in% c("c.A lot of difficulty", "b.Some difficulty", "d.Cannot do at all"), 1, 0)
# Create dummy variables for dis_dif_hands_s
individual_data$dis_dif_hands_dummy <- ifelse(individual_data$dis_dif_hands_s %in% c("c.A lot of difficulty", "b.Some difficulty", "d.Cannot do at all"), 1, 0)
# Create dummy variables for dis_dif_rais_jug_s
individual_data$dis_dif_rais_jug_dummy <- ifelse(individual_data$dis_dif_rais_jug_s %in% c("c.A lot of difficulty", "b.Some difficulty", "d.Cannot do at all"), 1, 0)
# Create a dummy variable for dis_dif_walking_s:
individual_data$dis_dif_walking_dummy <- ifelse(individual_data$dis_dif_walking_s %in% c("c.A lot of difficulty", "b.Some difficulty", "d.Cannot do at all"), 1, 0)
...
unique(individual_data$child_sick_yn)
...
names(individual_data)[grepl("health_care_imp_needs_s", names(individual_data))]

individual_data$chronic_illness_dummy <- ifelse(individual_data$health_care_imp_needs_s %in% c("k.Cancer treatment, including chemotherapy or radiotherapy (SHC)",
                                                               "i.Emergency, life saving surgery, including trauma care (SHC)",
                                                               "n.New born illness (SHC)",
                                                               "p.malnutrition treatment - inpatient (SHC)"), 1, 0)

table(individual_data$chronic_illness_dummy)
#needed: health_prob_acc_s
#received: health_obtain_care_s

#mental illness: "dis_child_learn_s", "dis_dif_remembering_s":
#create a dummy variable for dis_child_learn_s:
unique(individual_data$dis_child_learn_s)
individual_data$dis_child_learn_dummy <- ifelse(individual_data$dis_child_learn_s %in% c("c.A lot of difficulty", "b.Some difficulty", "d.Cannot do at all"), 1, 0)
#create a dummy variable for dis_dif_remembering_s:
individual_data$dis_dif_remembering_dummy <- ifelse(individual_data$dis_dif_remembering_s %in% c("c.A lot of difficulty", "b.Some difficulty", "d.Cannot do at all"), 1, 0)
...
table(individual_data$work_for_pay_yn)
individual_data$work_regular_dummy <- ifelse(individual_data$work_for_pay_yn %in% c("a. Yes"), 1, 0)
...
table(individual_data$child_and_work_regular)
...
table(household_data$lack_of_food_yn)
#turn lack_of_food_yn into a dummy variable:
household_data$lack_of_food_dummy <- ifelse(household_data$lack_of_food_yn %in% c("a. Yes"), 1, 0)

#turn red_food_exp_yn into a dummy variable:
household_data$eat_elsewhere_i <- ifelse(household_data$red_food_exp_yn %in% c("a. Yes"), 1, 0)
...
household_data$rCSI <- (household_data$less_expensive_i +
                          ...
                          3*household_data$lack_of_food_dummy +
...
table(household_data$bought_food_credit_yn)
...
#ASSETS: 
#use grepl to see the variables that have the name asset
names(household_data)[grepl("asset", names(household_data))]

#cut the assets_owned_ part of the variables
...
# asset_variables <- c("assets_owned_mattresses", "assets_owned_blankets", "assets_owned_winter_clothing", "assets_owned_beds",
#                      "assets_owned_table_chair", "assets_owned_small_gas_stove", "assets_owned_refrigerator", "assets_owned_overn",
#                      "assets_owned_ports_pans", "assets_owned_separate_freezer", "assets_owned_water_containers", "assets_owned_heater",
#                      "assets_owned_water_heater", "assets_owned_washing_machine", "assets_owned_air_condition", "assets_owned_sewing_machine",
#                      "assets_owned_tv", "assets_owned_computer", "assets_owned_satelite_dish", "assets_owned_mobile_phone",
#                      "assets_owned_internet", "assets_owned_motorcycle", "assets_owned_car_van_truck", "assets_owned_fishing_farming",
#                      "assets_owned_domestic_cleaning", "assets_owned_generator", "assets_owned_none", "assets_work_", "assets_work_sewing_machine",
#                      "assets_work_computer", "assets_work_satelite_dish", "assets_work_mobile_phone", "assets_work_internet",
#                      "assets_work_motorcycle", "assets_work_car_van_truck", "assets_work_fisihing_farming")
# 
# table(household_data$assets_work_internet)
...
# household_data1 <- household_data %>% select(formid, assets_owned_mattresses_dummy, assets_owned_blankets_dummy, assets_owned_winter_clothing_dummy, assets_owned_beds_dummy,
#                                              assets_owned_table_chair_dummy, assets_owned_small_gas_stove_dummy, assets_owned_refrigerator_dummy, assets_owned_overn_dummy,
#                                              assets_owned_ports_pans_dummy, assets_owned_separate_freezer_dummy, assets_owned_water_containers_dummy, assets_owned_heater_dummy,
#                                              assets_owned_water_heater_dummy, assets_owned_washing_machine_dummy, assets_owned_air_condition_dummy, assets_owned_sewing_machine_dummy,
#                                              assets_owned_tv_dummy, assets_owned_computer_dummy, assets_owned_satelite_dish_dummy, assets_owned_mobile_phone_dummy,
#                                              assets_owned_internet_dummy, assets_owned_motorcycle_dummy, assets_owned_car_van_truck_dummy, assets_owned_fishing_farming_dummy,
#                                              assets_owned_domestic_cleaning_dummy, assets_owned_generator_dummy, assets_owned_none_dummy, assets_work_sewing_machine_dummy,
#                                              assets_work_computer_dummy, assets_work_satelite_dish_dummy, assets_work_mobile_phone_dummy, assets_work_internet_dummy,
#                                              assets_work_motorcycle_dummy, assets_work_car_van_truck_dummy, assets_work_fisihing_farming_dummy)
# 
# #create total assets variable, that sums all the recent dummy variables:
# household_data1$total_assets <- rowSums(household_data1[, 2:36], na.rm = TRUE)
# 
# #create a basic_household_assets variable:
# # List of asset categories and corresponding dummy variables
# asset_categories <- list(
#   basic_household_assets = c("assets_owned_air_condition_dummy", "assets_owned_beds_dummy", "assets_owned_blankets_dummy", "assets_owned_mattresses_dummy", "assets_owned_winter_clothing_dummy"),
#   appliance_assets = c("assets_owned_heater_dummy", "assets_owned_washing_machine_dummy", "assets_owned_sewing_machine_dummy", "assets_owned_water_heater_dummy", "assets_owned_water_containers_dummy", "assets_owned_refrigerator_dummy")
# )
# 
...
# household_data$communication_assets <- rowSums(household_data[, c("assets_owned_tv_dummy", "assets_owned_satelite_dish_dummy", "assets_owned_mobile_phone_dummy", "assets_owned_internet_dummy", "assets_owned_computer_dummy")])
# 
# table(household_data$communication_assets)
# table(household_data$assets_owned_mobile_phone_dummy)
# #mutate assets_owned_mobile_phone_dummy to mobile_phone_dummy:
# household_data <- household_data %>% mutate(mobile_phone_dummy = assets_owned_mobile_phone_dummy)
# 
# #transportation_assets: 
# #create transportation_assets variable:
# household_data$transportation_assets <- rowSums(household_data[, c("assets_owned_motorcycle_dummy", "assets_owned_car_van_truck_dummy")])
...
#turn into dummy:
household_data$personal_hygiene_dummy <- ifelse(household_data$pers_hygiene_yn %in% c("a. Yes"), 1, 0)
#create female hygiene dummy, as 1-female_hygiene_m_1
household_data$female_hygiene_dummy <- ifelse(household_data$female_hygiene_m_1 %in% c("0"), 1, 0)

table(household_data$cleaning_items_yn)
#turn into dummy:
household_data$cleaning_items_dummy <- ifelse(household_data$cleaning_items_yn %in% c("a. Yes"), 1, 0)

table(household_data$baby_care_yn)
#turn into dummy:
household_data$baby_care_dummy <- ifelse(household_data$baby_care_yn %in% c("a. Yes"), 1, 0)

#mobile phone, internet, electricity access, wifi, make a table
table(household_data$smart_phone_yn)
#turn into dummy:
household_data$smart_phone_dummy <- ifelse(household_data$smart_phone_yn %in% c("a. Yes"), 1, 0)
table(household_data$have_internet_phone_yn)
#turn into dummy:
household_data$internet_phone_dummy <- ifelse(household_data$have_internet_phone_yn %in% c("a. Yes"), 1, 0)
table(household_data$electricity_access_yn)
#turn into dummy:
household_data$electricity_access_dummy <- ifelse(household_data$electricity_access_yn %in% c("a. Yes"), 1, 0)
...
#turn into dummies:
household_data$leaking_roof_dummy <- ifelse(household_data$leaking_roof_yn %in% c("a. Yes"), 1, 0)
household_data$damaged_shelter_dummy <- ifelse(household_data$damaged_shelter_yn %in% c("a. Yes"), 1, 0)
household_data$damaged_roof_dummy <- ifelse(household_data$damaged_roof_yn %in% c("a. Yes"), 1, 0)
household_data$damage_walls_dummy <- ifelse(household_data$damage_walls_yn %in% c("a. Yes"), 1, 0)
household_data$water_pipes_not_func_dummy <- ifelse(household_data$water_pipes_not_func_yn %in% c("a. Yes"), 1, 0)
household_data$rottenness_leak_dummy <- ifelse(household_data$rottenness_leak_yn %in% c("a. Yes"), 1, 0)
household_data$unsealed_win_doors_dummy <- ifelse(household_data$unsealed_win_doors_yn %in% c("a. Yes"), 1, 0)


# Create basic_household_assets variable, that sums "leaking_roof", "damaged_shelter", "damaged_roof", "damage_walls", "water_pipes_not_func_dummy", "rottenness_leak_dummy", "unsealed_win_doors_dummy":
household_data$basic_household_assets <- rowSums(household_data[, c("leaking_roof_dummy", "damaged_shelter_dummy", "damaged_roof_dummy", "damage_walls_dummy", "water_pipes_not_func_dummy", "rottenness_leak_dummy", "unsealed_win_doors_dummy")])
...
#communication_assets:
household_data$communication_assets <- rowSums(household_data[, c("electricity_access_dummy", "internet_phone_dummy")])

#mobile phone dummy, mutate smart_phone_dummy to mobile_phone_dummy:
household_data <- household_data %>% mutate(mobile_phone_dummy = smart_phone_dummy)
...
table(household_data$total_income_dec)
table(household_data$saving_exp_cash_dec)

#create a variable called total_expenditure that's 'total_income_dec - saving_exp_cash_dec:
household_data$expenditure_total <- as.numeric(household_data$total_income_dec) - as.numeric(household_data$saving_exp_cash_dec)

#summary of the variable:
summary(household_data$expenditure_total)
#how many na's are there in the variable:
sum(is.na(household_data$expenditure_total))
...
names(household_data)[grepl("condi", names(household_data))]

household_data$FCScore <- (ifelse(is.na(household_data$num_days_tubers_cons_i), 0, household_data$num_days_tubers_cons_i) * 3) + 
  (ifelse(is.na(household_data$num_days_cereals_i), 0, household_data$num_days_cereals_i) * 2) +
  ...
  (ifelse(is.na(household_data$num_days_condiments_i), 0, household_data$num_days_condiments_i) * 0)

summary(household_data$FCScore)

#curfew variables:
names(household_data)[grepl("prefer_rep_feedback_m6", names(household_data))]
table(household_data$curfew_imposed_yn)
household_data$curfew_dummy <- ifelse(household_data$curfew_imposed_yn == "a. Yes", 1, 0)

#technology access:
table(household_data$prefer_rep_feedback_m6) #most get their info from sms. 
household_data$info_servicessms_dummy <- household_data$prefer_rep_feedback_m6

#toilet sharing:
table(household_data$num_ppl_share_bathrooms_i)
#sharing_bathroom #number of people sharing bathroom
#shared_toilets #yes/no
household_data$shared_toilets_dummy <- ifelse(household_data$num_ppl_share_bathrooms_i > 1, 1, 0)

#arrival at same time:
table(household_data$women_feel_unsafe_s) #yes/no
#insecurity
household_data$insecurity_dummy <- ifelse(household_data$women_feel_unsafe_s == "a. Yes", 1, 0)
...
household_data$health_required1_dummy <- ifelse(household_data$cm_barriers_m_opts_2 == "1", 1, 0)

#prim_health_case_ass_yn : health access
household_data$health_access1_dummy <- ifelse(household_data$access_med_services_yn == "a. Yes", 1, 0)
...
#expenditure_food
#total_income
#first_income_sources
#calcexpenditure_food_usd

#mutate total_food_dec to expenditure_food:
household_data <- household_data %>% mutate(expenditure_food = total_food_dec)
...
write.csv(merged_df, "merged_df_2022.csv")
