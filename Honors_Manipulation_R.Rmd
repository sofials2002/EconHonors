...
#change to integer:
UNHCR_LBN_2018_VASYR.concise$household_id <- as.integer(UNHCR_LBN_2018_VASYR.concise$household_id)

#Now tackle error with hh size
UNHCR_LBN_2019_VASYR.concise$hh_size <- UNHCR_LBN_2019_VASYR$total_num_hh_i

UNHCR_LBN_2019_VASYR.concise <- UNHCR_LBN_2019_VASYR.concise %>%
mutate(hh_size = ifelse(household_size == "10 or more", "10", hh_size)) %>%
  
### ERROR!! HERE IT STOPS WORKING :(( CANNOT FIND THE 2019 ANYMORE :((

  mutate(hh_size = as.integer(hh_size))

# Now, convert the modified column to integer
your_data_frame$household_size <- as.integer(your_data_frame$household_size)
